...
class GCN(nn.Module):
    ...
    def _attentive_forward(self, y):
        # spatial attention
        se = y.mean(-2)  # N C V
        ms = self.sigmoid(self.conv_sa(se))
        y = y * ms.unsqueeze(-2) + y

        # temporal attention
        se = y.mean(-1)
        mt = self.sigmoid(self.conv_ta(se))
        y = y * mt.unsqueeze(-1) + y

        # channel attention
        se = y.mean(-1).mean(-1)
        se1 = self.relu(self.fc1c(se))
        mc = self.sigmoid(self.fc2c(se1))
        y = y * mc.unsqueeze(-1).unsqueeze(-1) + y
    ...
    def _adaptive_forward(self, x, y):
        ...
        for i in range(self.num_subset):            
            ...
            if self.masking:
                A1 = A1 * self.mask[i].to(x.device)

            if self.store_graphs:
                self.learned_graphs.append(A1.detach().cpu().numpy())
    ...
    def get_learned_graphs(self):
        return self.learned_graphs if hasattr(self, 'learned_graphs') else []
    
    def get_learned_attention(self):
        return self.learned_attention if hasattr(self, 'learned_attention') else []
...
class AAGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        y = self.relu(self.tcn(self.dropout(self.gcn(x))) + self.residual(x))
        return y

    @property
    def learned_graphs(self):
        return self.gcn.get_learned_graphs()
    
    @property
    def learned_attention(self):
        return self.gcn.get_learned_attention()
